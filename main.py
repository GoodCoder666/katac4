...
class OVInference:
    def __init__(self, model_path, height, width):
        ...
        model.reshape({"input": (1, 6, height, width)})
        self.compiled_model = core.compile_model(
            model, "CPU", {"PERFORMANCE_HINT": "LATENCY"}
        )
        self.request = self.compiled_model.create_infer_request()

        self.input_tensor = self.request.get_input_tensor()
        self.output_policy = self.request.get_output_tensor(0)
        self.output_value = self.request.get_output_tensor(1)

    def policy_value_fn(self, game):
        state = game.state()
        self.input_tensor.data[0] = state
        self.request.infer()
        policy_logits = self.output_policy.data.squeeze(0)
        value_logits = self.output_value.data.squeeze(0)
        ...
        policy = softmax(relevant_logits)
...
class GameSession:
    ...
    def _build_policy_fn(self, game):
        self._graph = OVInference(self.model_path, game.height, game.width)
